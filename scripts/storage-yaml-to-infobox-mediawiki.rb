...
storage_type = ARGV.shift()    # This might be 'st506' or 'dssi' etc.
storage_yaml = ARGV.shift()    # This is the storage YAML file
...
# Load the storage YAML information
storage = YAML.load_file(storage_yaml)

# Load the supplied tags
tags = DataTags.new(tags_yaml, 'storage', storage_type)
...
source_file_time = File.mtime(storage_yaml)

# storage will be a hash of {device name => properties-hash} 
...
storage.keys().each() {
  ...
  if build_xml
    ...
    op.puts_xml(%Q[      <comment>Created from #{File.basename(storage_yaml)}, last modified at #{source_file_time}</comment>])
  ...
  op.puts()
  op.puts("{{Infobox#{storage_type.upcase()}-Data")
  op.puts("| name = #{name}")
